...
class ProgramGenerator:
    ...
    def __init__(self,min_level, max_level, mpi_rank=0,cgs_level=0, use_mpi=False) -> None:
        ...
        self.template_path = f"{cwd}/Stage_Multigrille/build"
        self.problem = "run"
        self.uses_FAS = False
        # generate build path
    ...
    def traverse_graph(self, expression):
        ...
        if expr_type == "Cycle" and expression not in self.cycle_objs:
            ...
            cur_state['relaxation_factor'] = float(expression.relaxation_factor)
        ...
        if expr_type == "Multiplication":
            ...
            if op_type == "CoarseGridSolver":
                ...
                cur_state['relaxation_factor'] = 1.0
    ...
    def set_mginputs(self):
        ...
        while cur_lvl > first_state_lvl:
            ...
            self.relaxation_weights.append(0.0)
            ...
            if state['correction_type']==CorrectionTypes.Smoothing: # smoothing correction
                if state['component'] == Smoothers.CGS_GE:
                    ...
                    while cur_lvl > self.cgs_level:
                        ...
                        self.relaxation_weights.append(1.0)
                        self.intergrid_ops.append(InterGridOperations.Restriction)
                        self.cgc_weights.append(1)
                        cur_lvl -=1
    ...
    def generate_cmdline_args(self):
        ...
        for i in range(len(line)):
            if o in line[i]:
                line[i] = "operators = [" + list_to_string(self.intergrid_ops) + "]\n"
            if s in line[i]:
                line[i] = "smoothers = [" + list_to_string(self.smoothers) + "]\n"
            if w in line[i]:
                line[i] = "weights = [" + list_to_string(self.relaxation_weights) + "]"
    ...
    def execute_code(self, cmd_args=None):
        ...
        try:
            ...
            if self.use_mpi:
                ...
                a_lancer = [executable_path, input_file_path]
